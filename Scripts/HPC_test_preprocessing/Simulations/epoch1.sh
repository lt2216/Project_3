...
do
	echo $SEL
	# 1 epoch model
	java -jar $MSMS -N $NREF -ms $NCHROMS $NREPL -t $THETA -r $RHO $LEN -Sp $SELPOS -SI $SELTIME 1 $FREQ -SAA $(($SEL*2)) -SAa $SEL -Saa 0 -Smark -seed 12345 -thread 12 | gzip > NREPL="$1"/Simulations_1epoch/msms..$SEL..txt.gz
