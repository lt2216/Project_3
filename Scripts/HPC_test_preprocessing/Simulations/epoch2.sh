...
do
	echo $SEL
	# 2 epoch model
	java -jar $MSMS -N $NREF -ms $NCHROMS $NREPL -t $THETA -r $RHO $LEN -Sp $SELPOS -SI $SELTIME 1 $FREQ -SAA $(($SEL*2)) -SAa $SEL -Saa 0 -Smark $MARTH2 -seed 12345 -thread 12 | gzip > NREPL="$1"/Simulations_2epoch/msms..$SEL..txt.gz
done
